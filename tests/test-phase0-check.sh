...
echo "$output_empty" | jq -e '.hookSpecificOutput.message' >/dev/null || fail "empty case: no message field in hook output"
message_empty=$(echo "$output_empty" | jq -r '.hookSpecificOutput.message')
...
message_full=$(echo "$output_full" | jq -r '.hookSpecificOutput.message')
