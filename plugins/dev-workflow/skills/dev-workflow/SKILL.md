...
- `openspec` CLI is installed and `openspec/` exists in this repo.
- `bd` CLI is installed and `.beads/` exists in this repo.
- (Optional) `graphify` CLI is installed and `graphify-out/graph.json` exists, for codebase-graph-backed context — safe to skip if the repo is small.
