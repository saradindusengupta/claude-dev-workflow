...
Skills in this plugin are invoked with the `dev-workflow:` prefix — Claude Code namespaces skills by plugin name, so it's `/dev-workflow:dev-workflow` and `/dev-workflow:preflight`, not the bare names.
